(() => {
  ...
  function rawLines(value, lines = [], level = 0, indexWidth = 1) { const indent = '\t'.repeat(level); if (!Array.isArray(value)) { lines.push(`${indent}${value}`); return lines; } value.forEach((item, index) => { const indexText = String(index); const bracketedIndex = `[${indexText}]`; const maxBracketWidth = indexWidth + 2; const equalsPadding = ' '.repeat(Math.max(1, maxBracketWidth - bracketedIndex.length + 1)); if (Array.isArray(item)) lines.push(`${indent}${bracketedIndex}${equalsPadding}=`); else lines.push(`${indent}${bracketedIndex}${equalsPadding}= ${item}`); if (Array.isArray(item)) rawLines(item, lines, level + 1, indexWidth); }); return lines; }
  ...
  function parseAndRender() { const input = document.getElementById('input'), output = document.getElementById('output'), jsonRadio = document.getElementById('deOutputJson'); if (!input || !output) return; try { const parsed = parse(input.value); output.textContent = jsonRadio?.checked ? formatJson(parsed) : formatRaw(parsed); const packetTitle = document.getElementById('packetTitle'), meta = document.getElementById('meta'); if (packetTitle) packetTitle.textContent = 'DE Parser Result'; if (meta) meta.textContent = `${input.value.length} characters`; } catch (error) { output.textContent = `DE Parser Error: ${error.message}`; } }
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    button.addEventListener('click', () => { modeRow.querySelectorAll('.mode').forEach(b => b.classList.remove('active')); button.classList.add('active'); setDEMode(true); parseAndRender(); });
    ...
    // DE data is not necessarily HEX. Capture the input event before app.js's ISO HEX validator.
    document.getElementById('input')?.addEventListener('input', event => {
      if (document.querySelector('.mode.active')?.dataset.mode !== 'de') return;
      event.stopImmediatePropagation();
      clearTimeout(window.__deAutoParseTimer);
      window.__deAutoParseTimer = setTimeout(parseAndRender, 300);
    }, true);
  });
})();
